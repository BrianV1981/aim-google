...
`aim-google contacts update` supports JSON input via `--from-file`, so you can update People API fields without adding new CLI flags.
...
```bash
aim-google contacts get people/c123456 --json > contact.json

# Edit contact.json (see notes below)
aim-google contacts update people/c123456 --from-file contact.json
```

Update from stdin:

```bash
aim-google contacts get people/c123456 --json | \
  jq '(.contact.urls //= []) | (.contact.urls += [{"value":"https://example.com","type":"profile"}])' | \
  aim-google contacts update people/c123456 --from-file -
```
...
- Wrapped (from `aim-google contacts get --json`): `{"contact": { ...person... }}`
...
If the JSON contains unsupported fields (for `updateContact`), aim-google errors instead of silently ignoring them.
...
To avoid overwriting concurrent contact edits, aim-google compares the JSON etag with the current contact etag:
