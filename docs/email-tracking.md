---
summary: "Email open tracking in aim-google (Gmail + Cloudflare Worker)"
read_when:
...
Goal: track email opens for `aim-google gmail send` via a tiny tracking pixel served from a Cloudflare Worker.

High-level:
- `aim-google gmail send --track` injects a 1×1 image URL into the HTML body.
- The Worker receives the request, stores an “open” row in D1, and returns a transparent pixel.
- `aim-google gmail track opens …` queries the Worker and prints opens.
...
```sh
aim-google gmail track setup --worker-url https://aim-google-email-tracker.<acct>.workers.dev --deploy
```

Flags:
- `--worker-name`: default `aim-google-email-tracker-<account>`.
- `--db-name`: default to worker name.
- `--worker-dir`: default `internal/tracking/worker`.

Re-run `aim-google gmail track setup` any time to re-print the current `TRACKING_KEY` / `ADMIN_KEY` values (it’s idempotent unless you pass explicit `--tracking-key` / `--admin-key`).
...
Provision secrets (use values printed by `aim-google gmail track setup`):
...
```sh
pnpm exec wrangler d1 create aim-google-email-tracker
...
- `required: --worker-url`: run `aim-google gmail track setup --worker-url …` first (or pass `--worker-url` again).
