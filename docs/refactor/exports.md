...
- `aim-google docs export <docId> --format pdf|docx|txt|md|html`
- `aim-google slides export <presentationId> --format pdf|pptx`
- `aim-google sheets export <spreadsheetId> --format pdf|xlsx|csv`
...
- `--out` defaults to `$(os.UserConfigDir())/aim-google/drive-downloads/` (via `internal/config:EnsureDriveDownloadsDir`).
