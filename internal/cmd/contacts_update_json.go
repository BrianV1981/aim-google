...
func parseContactsUpdateJSON(data []byte) (*people.Person, map[string]json.RawMessage, error) {
	...
	// Support wrapped format from `aim-google contacts get --json`: {"contact": {...}}.
...
func contactsUpdateMaskFromKeys(keys map[string]json.RawMessage) ([]string, error) {
	...
	if len(unsupported) > 0 {
		...
		return nil, usage("JSON contains unsupported keys for contacts update: " + strings.Join(unsupported, ", ") + ". Include only fields you want to change (for example: urls, biographies, addresses, organizations, ...). Tip: start from `aim-google contacts get ... --json` and delete keys you don't want to update.")
	}
	sort.Strings(update)
	return update, nil
}
...
func (c *ContactsUpdateCmd) updateFromJSON(ctx context.Context, svc *people.Service, resourceName string, u *ui.UI) error {
	...
	if inputETag == "" {
		u.Err().Println("warning: JSON input is missing an etag; consider starting from `aim-google contacts get ... --json`")
	} else if !c.IgnoreETag && curETag != "" && inputETag != curETag {
		return usage("etag mismatch (contact changed). Re-run `aim-google contacts get ... --json`, re-apply edits, retry (or pass --ignore-etag).")
	}
