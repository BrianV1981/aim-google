...
func TestLiveScript(t *testing.T) {
	if os.Getenv("AIM_GOOGLE_LIVE") == "" {
		t.Skip("set AIM_GOOGLE_LIVE=1 to run live tests")
	}
	...
	args := []string{}
	if os.Getenv("AIM_GOOGLE_LIVE_FAST") != "" {
		args = append(args, "--fast")
	}
	if os.Getenv("AIM_GOOGLE_LIVE_STRICT") != "" {
		args = append(args, "--strict")
	}
	if v := firstNonEmpty(os.Getenv("AIM_GOOGLE_LIVE_ACCOUNT"), os.Getenv("AIM_GOOGLE_IT_ACCOUNT")); v != "" {
		args = append(args, "--account", v)
	}
	if v := os.Getenv("AIM_GOOGLE_LIVE_SKIP"); v != "" {
		args = append(args, "--skip", v)
	}
	if v := os.Getenv("AIM_GOOGLE_LIVE_AUTH"); v != "" {
		args = append(args, "--auth", v)
	}
	if os.Getenv("AIM_GOOGLE_LIVE_ALLOW_NONTEST") != "" {
