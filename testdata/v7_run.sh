...
# Step 0: Clear
echo "--- Clearing ---"
$AIM_GOOGLE docs sed "$DOC" -a "$ACCT" 's/.*\n//g' 2>/dev/null || true
for i in $(seq 1 20); do
    $AIM_GOOGLE docs sed "$DOC" -a "$ACCT" 's/|1|//' 2>&1 | grep -q "out of range\|no tables" && break
done 2>/dev/null
$AIM_GOOGLE docs sed "$DOC" -a "$ACCT" 's/.*\n//g' 2>/dev/null || true
sleep 2
...
SEED_EXPR="${SEED_EXPR%\\n}/"
$AIM_GOOGLE docs sed "$DOC" -a "$ACCT" "$SEED_EXPR"
sleep 2

# Step 2: Main formatting
echo "--- v7_test.sed ---"
grep -v '^#' "$DIR/v7_test.sed" | grep -v '^$' | $AIM_GOOGLE docs sed "$DOC" -a "$ACCT"
sleep 3

# Step 3: Table operations
echo "--- v7_table_ops.sed ---"
grep -v '^#' "$DIR/v7_table_ops.sed" | grep -v '^$' | $AIM_GOOGLE docs sed "$DOC" -a "$ACCT"
